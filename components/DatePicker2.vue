<script setup>
	...
	const options = {
		...
		beforeShowDay: function (date) {
			// List of specific dates to disable (in ISO format)
			const disabledDates = ["2024-04-01", "2024-04-05", "2024-04-10"];
			// Convert current date to ISO format for comparison
			const isoDate = date.toISOString().slice(0, 10);
			// Check if the current date is in the disabledDates array
			return { disabled: disabledDates.includes(isoDate) };
		},
	};
	...
	onMounted(() => {
		...
		airDatepicker.$datepicker.style.width = screenWidth > 768 && "350px";
		airDatepicker.$datepicker.style.height = "auto";
	...
	function nextClicked() {
		if (selectedDate.value !== null && selectedTimeSlotIndex.value !== null) {
			selectedTimeSlot.value = timeSlots.value[selectedTimeSlotIndex.value];
			nextTab();
		} else {
	...
	const isSlotDisable = (index) => {
		return disabledSlots.value.includes(timeSlots.value[index].start);
	};
</script>
...
<template>
	<div>
		...
		<div v-show="currentTab === 1">
			<div class="md:flex items-center justify-end gap-6">
				<div v-if="!selectedDate || !isSmallDevice">
					<div id="my-element" inline-datepicker></div>
				</div>
				<!-- time section -->
				<div v-if="selectedDate" class="w-[250px] px-2 py-5">
					<p class="text-center text-xl font-bold md:text-base md:font-normal">
						{{ formattedDate }}
					</p>
					...
					<div class="mt-2 md:h-[268px] overflow-y-scroll no-scrollbar">
						<div
							...
							<div
								...
								@click="
									isSlotDisable(index) ? () => {} : selectTimeSlot(index)
								">
								{{ slot.start }}
							</div>
							<transition name="fade">
								<div
									v-if="selectedTimeSlotIndex === index"
		...
		<div
			...
			<div class="w-full max-w-lg">
				...
				<div
					...
					class="fixed top-5 left-2 md:hidden">
					<button
				...
				<div
					@click="prevTab"
					:disabled="currentTab === 1"
					class="absolute top-5 left-2 hidden md:block">
					<button
						class="w-fit p-1.5 rounded-full border text-blue-500 hover:text-blue-600">
						<svg
							xmlns="http://www.w3.org/2000/svg"
		...
		<div class="px-5 md:px-0 py-10 md:py-2" v-show="currentTab === 3">
			...
			<p class="text-gray-600 my-3 text-center md:text-left">
			...
			<div class="border rounded-xl p-3 my-5">
				...
				<div class="flex items-center text-gray-600 mt-1">
					...
					<p class="font-semibold">
						<span>{{ selectedTimeSlot && selectTimeSlot.start }}, </span
						>{{ formattedDate }}
					</p>
				</div>
				<div class="flex items-center text-gray-600 mt-1">
